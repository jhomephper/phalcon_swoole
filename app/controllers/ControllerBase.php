...
class ControllerBase extends Controller
{
    ...
    public function initialize() {
        ...
        //后台查看权限约束
        if(!$this->cookies->get('username')->getValue()){
     ...
     * @return string
     */
    protected static function getMessage($code = 0)
    {
    ...
    {
        if (isset(self::$statusCode[$status]))
            return self::$statusCode[$status];
        else
            return 'Unknown status.';
    }

    protected function response($status, $code, $data = '')
    {
        $message = self::getMessage($code);
        $statusMsg = self::getStatus($status);
        $this->app->response
            ->setStatusCode($status, $statusMsg)
            ->setJsonContent(compact('code', 'message', 'data'));
    }
